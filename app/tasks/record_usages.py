...
def record_user_usage_logs(
    db: Session, params: list, node_id: int, consumption_factor: int = 1
):
    if not params:
        return
...
def record_node_stats(db: Session, node_id: int, usage: int):
    if not usage:
        return
...
async def record_user_usages():
    ...
    with GetDB() as db:
        ...
        for node_id, params in api_params.items():
            ...
            record_node_stats(db, node_id, node_usage)
